...
void P_Bullet::setColor(Uint8 red, Uint8 green, Uint8 blue)
{
	SDL_SetTextureColorMod(B_Texture, red, green, blue);
}
void P_Bullet::setBlendMode(SDL_BlendMode blending)
{

	SDL_SetTextureBlendMode(B_Texture, blending);
}
void P_Bullet::setAlpha(Uint8 alpha)
{
	SDL_SetTextureAlphaMod(B_Texture, alpha);
}
bool P_Bullet::loadFromFile(string path) {
...
void P_Bullet::render(int x, int y, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip)
{
	exist = true;
